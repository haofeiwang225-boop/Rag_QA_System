...
def _resolve_bge_runtime(model_device: str, use_fp16: bool):
    device = model_device or "cpu"

    if device != "cpu":
        try:
            if embedding_config.bge_backend_import:
                importlib.import_module(embedding_config.bge_backend_import)
                logger.info(f"已导入BGE后端注册包：{embedding_config.bge_backend_import}")

            import torch

            if device.startswith("cuda") and not torch.cuda.is_available():
                logger.warning(f"BGE_DEVICE={device}不可用，自动回退到cpu")
                device = "cpu"
            ...
            logger.warning(f"无法校验BGE_DEVICE={device}：{str(e)}，自动回退到cpu")
            device = "cpu"

    if device == "cpu" and use_fp16:
        logger.warning("BGE模型运行在cpu上，已自动关闭fp16")
        use_fp16 = False
...
def get_bge_m3_ef():
    ...
    device, use_fp16 = _resolve_bge_runtime(
        embedding_config.bge_device, embedding_config.bge_fp16 or False
    )
