...
from app.clients.minio_utils import get_minio_client, minio_client
...
def find_image_in_md(md_content, image_filename, context_len=100):
    """
...
def step_2_scan_images(md_content, images_dir_obj:Path) :
    targets = []
    ...
    for image_file in os.listdir(images_dir_obj):
        ...
        image_path = images_dir_obj / image_file
        ...
        targets.append((image_file,image_path,context_list))
...
def step_3_generate_summaries(doc_stem: str, targets: List[Tuple[str, str, Tuple[str, str]]],
    ...
    summaries = {}
    request_times = deque()
    for img_file, image_path, context_list in targets:
        apply_api_rate_limit(request_times, requests_per_minute, window_seconds=60)
        logger.debug(f"开始生成图片摘要：{image_path}")
        image_context = context_list[0] if context_list else ("", "")
        summaries[img_file] = summarize_image(image_path, root_folder=doc_stem, image_content=image_context)
    return summaries


def clean_minio_directory(minio_client, prefix):
    try:
...
def upload_to_minio(minio_client, local_path, object_name):
    """
...
def upload_images_batch(minio_client, upload_dir, targets):
    """
...
def merge_summary_and_url(summaries, urls):
    ...
    for image_file, summary in summaries.items():
        if url := urls.get(image_file):
            images_info[image_file] = (summary, url)

    return images_info


def process_md_file(md_content, image_info):
    """
    ...
    for img_filename, (summary, new_url) in image_info.items():
        rep = re.compile(r"!\[.*?\]\([^)]*" + re.escape(img_filename) + r"[^)]*\)",
                         re.IGNORECASE)

        md_content = rep.sub(f"![{summary}]({new_url})", md_content)
...
def step_5_backup_new_md_file(origin_obj_path,new_md_content):
    """
        ...
        :return: 新MD文件的完整路径
        """
    new_md_file_name = os.path.splitext(origin_obj_path)[0]+"_new.md"
