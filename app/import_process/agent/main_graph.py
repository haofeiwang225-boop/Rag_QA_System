...
from app.import_process.agent.state import ImportGraphState, create_default_state, graph_default_state
...
def router_after_entry(state: ImportGraphState)-> str:
    if state["is_pdf_read_enabled"]:
        return "is_pdf_read_enabled"
    elif state["is_md_read_enabled"]:
        return "is_md_read_enabled"
    else:
        return END

workflow.add_conditional_edges('node_entry',
                               router_after_entry,
                     {"is_pdf_read_enabled" : "node_pdf_to_md",
